...
A PR raised through the pipeline carries the deterministic signature that no-mistakes writes in the body.
Automation such as GitHub Actions and Dependabot is exempt so it keeps working, but regular contributor PRs without that signature will not be reviewed or merged.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, and pinned shellcheck version), and the no-mistakes pre-push gate runs it, so every caller lints the same surface the same way.
...
That includes documentation-only changes here: this repository's standing rigor is one the executed-words fast path in tracked [`doctrine/captain-principles.md`](doctrine/captain-principles.md) never lowers, and a PR raised outside the pipeline is not merged.
...
Local `.no-mistakes/` state and test evidence stay out of this repo; `.no-mistakes.yaml` keeps evidence in a temp directory and pins the gate's lint command to `bin/fm-lint.sh`.
Local no-mistakes Test is intent-targeted and must not re-run every `tests/*.test.sh`; the broad behavior suite and the platform-specific compatibility lanes are run through `bin/fm-test-run.sh`, not by any workflow in this repository.
...
```sh
while IFS= read -r script; do /bin/bash -n "$script" || exit; done < <(bin/fm-lint.sh --list-files)   # syntax-check the shell surface fm-lint.sh will cover (changed files locally)
bin/fm-lint.sh   # lint that same surface; the no-mistakes gate runs the same command
...
Broad regression across the portable parallel shards, the portable serial lane's separate-runner shards, the Herdr lane, lint, invariants, the coverage guard, and stock macOS Bash compatibility is run through `bin/fm-test-run.sh`; the only workflow this repository still runs is [`.github/workflows/app.yml`](.github/workflows/app.yml), which type-checks, lints, tests and builds the app under `app/`.
