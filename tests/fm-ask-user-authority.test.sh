#!/usr/bin/env bash
# Behavioral regressions for the ask-user authority instructions a worker is
# launched with. A ship worker gets them through the standing rules that
# bin/fm-spawn.sh composes onto its brief, so they are asserted on that composed
# prompt; a secondmate charter still carries its own text.
set -u
...
test_primary_and_secondmate_instruction_generation() {
  local home ship charter composed
  home="$TMP_ROOT/home"
  mkdir -p "$home/data"

  FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$BRIEF" authority-worker sample --mode no-mistakes >/dev/null 2>&1
  ship="$home/data/authority-worker/launch-prompt.md"
  fm_worker_rules_compose "$home/data/authority-worker/brief.md" authority-worker \
